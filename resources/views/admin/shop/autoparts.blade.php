        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
            ...
            <table class="table-fixed w-full">
                ...
                <tr class="bg-gray-100">
                    ...
                    <th class="px-6 py-3 text-center border-b-2 border-gray-300 text-left text-sm leading-4 tracking-wider">Срок<br>поставки</th>
                ...
                @foreach($autoparts as $autopart)
                    <tr>
                        <td class="px-6 py-4 border-b border-gray-300 text-sm leading-5">{{ $autopart->id }}</td>
                        <td class="px-6 py-4 border-b border-gray-300 text-sm leading-5">{{ Date::parse($autopart->created_at)->format('j F Y') }}</td>
                        <td class="px-6 py-4 border-b border-gray-300 text-sm leading-5">{{ $autopart->title }}</td>
                        <td class="px-6 py-4 border-b border-gray-300 text-sm leading-5">№{{ $autopart->number}}</td>
                        <td class="px-6 py-4 border-b border-gray-300 text-sm leading-5">{{ $autopart->price }} руб.</td>
                        <td class="px-6 py-4 border-b border-gray-300 text-sm leading-5">{{ $autopart->delivery_time }} дн.</td>
                        <td class="px-6 py-4 border-b border-gray-300 text-sm leading-5">
                            <button wire:click="edit({{ $autopart->id }})" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                                Редактировать
                            </button>
                            <button wire:click="delete({{ $autopart->id }})" class="mt-2 bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">
                                Удалить
                            </button>
                        </td>
                    </tr>
